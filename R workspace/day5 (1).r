...
table(ActualValue=training$type,PredictedValue = res4>0.3)     
plot(ROCRPref,colrize=TRUE,print.cutoffs.at=seq(0.1,by=0.1))   
res1<-predict(model1,training,type="response")
table(ActualValue=training$type,PredictedValue = res1>0.5)
plot(ROCRPref,colrize=TRUE,print.cutoffs.at=seq(0.1,by=0.1))
res5<-predict(model1,training,type="response")
ROCRPred <-prediction(res5,training$type)
ROCRPref<-performance(ROCRPred,"tpr","fpr")
table(ActualValue=training$type,PredictedValue = res5>0.2)
plot(ROCRPref,colrize=TRUE,print.cutoffs.at=seq(0.1,by=0.1))
#Threshold point should be 0.30
#Testing
model2<- glm(type ~.,testing,family="binomial")
summary(model)
restest<-predict(model2,testing,type="response")
ROCRPred <-prediction(restest,testing$type)
ROCRPref<-performance(ROCRPred,"tpr","fpr")
plot(ROCRPref,colrize=TRUE,print.cutoffs.at=seq(0.1,by=0.1))

table(ActualValue=testing$type,PredictedValue = restest>0.3)
table(ActualValue=testing$type,PredictedValue = restest>0.5)
table(ActualValue=testing$type,PredictedValue = restest>0.4)
